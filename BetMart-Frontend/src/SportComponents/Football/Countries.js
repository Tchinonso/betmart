...
export default function Countries({ passLeagues }) {
  ...
  const ShowLeagues = async (id, sport) => {
    await axios.post(`https://betmartoffical.herokuapp.com/api/sport/league`,{
      sport_name : sport,
      countryId : id
    })
    .then((response)=>{
      ...
      <div className="soccer-container">
        ...
        {countries &&
          countries.map((country) => (
            <div key={country.country_key} className="countries-sports" >
              <div
                className="countries-sports-contents"
                onClick={() => ShowLeagues(country.country_key, "football")}
              >
                <div className="logo">
